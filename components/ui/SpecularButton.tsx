...
type SpecularButtonAsButton = SpecularButtonBaseProps & {
  href?: undefined;
  type?: "button" | "submit" | "reset";
  onClick?: MouseEventHandler<HTMLButtonElement>;
};

type SpecularButtonAsLink = SpecularButtonBaseProps & {
  href: string;
  type?: never;
...
function useSpecularEffect(
  hostRef: RefObject<HTMLElement | null>,
  fxRef: RefObject<HTMLSpanElement | null>,
  propsRef: MutableRefObject<SpecularRuntimeProps>,
) {
  useEffect(() => {
    const host = hostRef.current;
    const fx = fxRef.current;
    if (!host || !fx || prefersReducedMotion() || !canUseWebGL()) return;
    ...
    try {
      ...
      // Unsupported WebGL — keep the styled button without the rim FX
      return;
    }
...
export function SpecularButton({
  ...
  const propsRef = useRef<SpecularRuntimeProps>({
    radius,
    lineColor,
    baseColor,
    intensity,
  ...
  if ("href" in props && props.href) {
    const guardClick: MouseEventHandler<HTMLAnchorElement> = (event) => {
      if (disabled) {
    ...
    if (isExternalHref(props.href)) {
      ...
      return (
        <a
          ref={hostRef as RefObject<HTMLAnchorElement>}
          href={props.href}
          className={classes}
          style={style}

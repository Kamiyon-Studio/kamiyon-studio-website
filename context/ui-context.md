...
| Page | Layout |
| --- | --- |
| Home | Combined full-bleed opening stage — **4-plate layered parallax** from R2 (`v2`: sky+ocean as WebP freeze-frame + WebM/MP4 masked to WebP alpha, mountain still, planted foreground; ADR-029 + ADR-033) with the original single-plate hero as fallback; brand + motto upper; **continuous partners logo marquee** lower — larger logos, grayscale→color on hover, `/#home-partners`, `data-nav-theme="dark"`, ADR-023 + ADR-026) → **recent projects** on the earth plate (`background.webp`, `#home-projects`) → **recognition awards** (unbounded CMS `awards` refs; LaurelBadge `placeholderLabel`) → **testimonials** (`/#home-testimonials`, ADR-031; CMS `testimonials` refs; empty CMS uses team-roster **preview quotes**; consented client quotes replace them; centered pill header overlay; full-bleed 3D four-column CSS marquee, extra columns hide at md/lg; marquee at ≥1; own `--bg-primary` section after awards; `--bg-secondary` trust chapter with awards) → **services vertical marquee** (`/#home-services`, ADR-021; CMS `services` refs) → contact CTA (`contactCta`). Home CMS is named fields — not `blocks[]` (ADR-030). Empty refs = nothing; CMS null = placeholders **except testimonials (team-roster preview until consented quotes exist)**. |
...
| Home testimonials | Four-column 3D CSS marquee (`perspective` + `animate-marquee-vertical`); static unique cards under reduced motion | `components/ui/testimonial-marquee` via `TestimonialsMarquee` (ADR-031) |
| Home hero layered parallax | 4 plates (sky/ocean play WebM or MP4 masked to the WebP freeze-frame; mountain still; foreground planted at `yPercent` 0); brand rides the depth-3 slot; earth plate is the Recent Projects backdrop; fine-pointer + motion-allowed only; reduced-motion hides the videos | `hooks/useLayeredParallax` via `HeroParallaxOpening` (ADR-029, ADR-033) |

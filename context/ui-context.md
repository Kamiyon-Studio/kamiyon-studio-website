...
| Page | Layout |
| --- | --- |
| Home | Combined full-bleed opening stage — **2-plate layered parallax** from R2 (`v3`: opaque landscape `homepage.mp4` with `fallback.avif` freeze-frame, planted `foreground.avif`; ADR-029 + ADR-033) with the original single-plate hero as fallback; brand + motto upper; **continuous partners logo marquee** lower — larger logos, grayscale→color on hover, `/#home-partners`, `data-nav-theme="dark"`, ADR-023 + ADR-026) → **recent projects** on the earth plate (`ground.avif`, `#home-projects`) → **recognition awards** (unbounded CMS `awards` refs; LaurelBadge `placeholderLabel`) → **services vertical marquee** (`/#home-services`, ADR-021; CMS `services` refs) → contact CTA (`contactCta`). **TestimonialsMarquee is hidden** on Home (ADR-035; component + CMS `testimonials[]` kept). Home CMS is named fields — not `blocks[]` (ADR-030). Empty refs = nothing; CMS null = placeholders. |
...
| Home hero layered parallax | 2 plates (landscape plays MP4 over `fallback.avif`; foreground planted at `yPercent` 0); plates pin `object-top` to the viewport; brand sits between them; planted cliff hangs `18svh` into Recent Projects so `ground.avif` tucks under and fades in; fine-pointer + motion-allowed only; reduced-motion hides the video | `hooks/useLayeredParallax` via `HeroParallaxOpening` (ADR-029, ADR-033) |

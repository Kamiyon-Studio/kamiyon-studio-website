...
| Page | Layout |
| --- | --- |
| Home | Combined full-bleed opening stage — **4-plate layered parallax** backdrop from R2 with the original single-plate hero as fallback (ADR-029); brand + motto upper; **continuous partners logo marquee** lower — larger logos, grayscale→color on hover, `/#home-partners`, `data-nav-theme="dark"`, ADR-023 + ADR-026) → projects bento (CMS `portfolioItems` refs) → **recognition awards** (unbounded CMS `awards` refs; LaurelBadge `placeholderLabel`) → **testimonials** (`/#home-testimonials`, ADR-031; CMS `testimonials` refs; empty or CMS-null **hides**; 1–2 static cards, 3+ vertical marquee; `--bg-secondary` trust chapter with awards) → **services vertical marquee** (`/#home-services`, ADR-021; CMS `services` refs) → contact CTA (`contactCta`). Home CMS is named fields — not `blocks[]` (ADR-030). Empty refs = nothing; CMS null = placeholders **except testimonials (always hide)**. |
| About | Full-viewport `ABOUT US` hero (`data-nav-theme="dark"`, homepage-like bg/parallax/opening) → two-column `OUR STORY` editorial intro → **WHO WE ARE** OurStory-style two-column band (`/#who-we-are`, Mission/Vision/Motto/Values/Culture/Team intro; ADR-030) → **Kamiyon milestone timeline** (`/#timeline`, ADR-025) → team **FocusRail** 3D carousel with click-to-expand card modal (`/#team`, ADR-028) — important section titles use **WordPullUp** / `DISPLAY_HEADING_CLASS` (Projects-style). Archived VisionBand / ValuesGrid / CultureClosing **not restored** (ADR-027 archive + ADR-030) |

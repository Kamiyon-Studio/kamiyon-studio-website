...
| Page | Layout |
| --- | --- |
| Home | Combined full-bleed opening stage — **2-plate layered parallax** from R2 (`v3`: opaque landscape `homepage.mp4` with `fallback.avif` freeze-frame, planted `foreground.avif`; ADR-029 + ADR-033) with the original single-plate hero as fallback; brand + motto (`/#home-hero`) → **recent projects** on the earth plate (`ground.avif`, `#home-projects`) → **partners logo marquee** as a standalone section (`/#home-partners`, Trusted by eyebrow, grayscale→color on hover, ADR-026 + ADR-036) → **recognition awards** (unbounded CMS `awards` refs; LaurelBadge `placeholderLabel`) → **services vertical marquee** (`/#home-services`, ADR-021; CMS `services` refs) → contact CTA (`contactCta`). **TestimonialsMarquee is hidden** on Home (ADR-035; component + CMS `testimonials[]` kept). Home CMS is named fields — not `blocks[]` (ADR-030). Empty refs = nothing; CMS null = placeholders. |
| About | Full-viewport `ABOUT US` hero (`data-nav-theme="dark"`, homepage-like bg/parallax/opening) → two-column `OUR STORY` editorial intro → **Kamiyon milestone timeline** (`/#timeline`, ADR-025) → team **FocusRail** 3D carousel with click-to-expand card modal (`/#team`, ADR-028) — important section titles use **WordPullUp** / `DISPLAY_HEADING_CLASS` (Projects-style). **WhoWeAreBand is hidden** on `/about` (ADR-034; component + CMS fields kept). Archived VisionBand / ValuesGrid / CultureClosing **not restored** (ADR-027 archive + ADR-030) |
...
| Home services discovery | Vertical text marquee (JS loop, pointer-draggable); each row is a link to `/services/{slug}`; hover reveal uses portfolio photos tagged with that service; static list under reduced motion | `components/ui/cta-with-text-marquee` via `ServicesStack` (ADR-021) |
| Home testimonials | **Hidden on Home** (ADR-035). Component kept: four-column 3D CSS marquee (`perspective` + `animate-marquee-vertical`); static unique cards under reduced motion | `components/ui/testimonial-marquee` via `TestimonialsMarquee` (ADR-031) |
| Home hero layered parallax | 2 plates (landscape plays MP4 over `fallback.avif`; foreground planted at `yPercent` 0); plates pin `object-top` to the viewport; brand sits between them with a subtle primary glyph glow (0-offset wrapper filter; no charcoal drop-shadow scrim); hanging soil dissolves to charcoal over `18svh` only (first viewport stays solid); fine-pointer + motion-allowed only; reduced-motion hides the video | `hooks/useLayeredParallax` via `HeroParallaxOpening` (ADR-029, ADR-033) |
